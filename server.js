...
io.on('connection', async (socket) => {
    ...
    socket.on('ack_message', async (payload) => {
        ...
        // Send updated pending count to all clients
        const pendingCount = await getPendingCount();
    ...
    socket.on('command', async (payload) => {
        ...
        if (['START', 'STOP', 'PAUSE'].includes(action)) {
            ...
            console.log(`📡 Broadcasting machine_command [${action}] to ${io.engine.clientsCount} clients [seq=${entry.seq}]`);
            io.emit('machine_status', {
        ...
        if (action === 'data_change') {
            ...
            console.log(`📡 Broadcasting data_changed [${changeType}] to ${io.engine.clientsCount} clients [seq=${entry.seq}]`);
    ...
    // ═══════════════════════════════════════════════════
    // ★★★ Direct event handling with io.emit ★★★
    // ═══════════════════════════════════════════════════
    ...
    async function handleDirectEvent(eventName, data, socket) {
        ...
        console.log(`📡 Broadcasting ${eventName} to ${io.engine.clientsCount} clients [seq=${entry.seq}, uuid=${entry.uuid.substring(0, 8)}...]`);
    ...
    socket.on('speed_update', async (data) => {
        ...
        console.log(`📡 Broadcasting speed_update [speed=${data.speed}]`);
        io.emit('speed_update', broadcastData);
    });
...
async function startServer() {
    ...
    server.listen(PORT, () => {
        ...
        console.log(`📡 io.emit() → ALL clients (broadcast fix)`);
