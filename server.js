...
io.on('connection', async (socket) => {
    ...
    socket.on('command', async (payload) => {
        ...
        if (['START', 'STOP', 'PAUSE'].includes(action)) {
            ...
            // ★★★ FIX: io.emit → ALL clients including sender ★★★
    ...
    // ═══════════════════════════════════════════════════════
    // ★★★ FIXED: handleDirectEvent — io.emit to ALL ★★★
    // ═══════════════════════════════════════════════════════
    ...
    async function handleDirectEvent(eventName, data, socket) {
        ...
        // ★★★ CORE FIX: io.emit instead of socket.broadcast.emit ★★★
        // This sends to ALL connected clients including sender
        const broadcastData = {
            ...cleanData,
            seq: entry.seq, uuid: entry.uuid, hash: entry.hash,
            _sender: socket.id
        ...
        // ★ ACK to sender with client UUID
        socket.emit('server_ack', {
            seq: entry.seq,
    ...
    socket.on('speed_update', async (data) => {
        ...
        // ★★★ FIX: io.emit for speed too ★★★
        const broadcastData = {
            speed: data.speed,
            seq: entry.seq, uuid: entry.uuid, hash: entry.hash,
            _sender: socket.id
        };
        console.log(`📡 Broadcasting speed_update to ${io.engine.clientsCount} clients [speed=${data.speed}]`);
...
async function startServer() {
    ...
    server.listen(PORT, () => {
        ...
        console.log(`🚀 السيرفر V14-FIX-BROADCAST جاهز على المنفذ ${PORT}`);
        console.log(`💾 PostgreSQL: ${dbAvailable ? '✅' : '❌'}`);
        console.log(`🔐 UUID + Server-side Hash`);
        console.log(`📡 BROADCAST FIX: io.emit() → ALL clients`);
