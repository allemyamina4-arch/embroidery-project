...
// ═══════════════════════════════════════════════════════
// ★★★ FIX #2: Stable WebSocket — tuned timeouts ★★★
// ═══════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════
// ★★★ FIX #1 & #4: Database Schema with ack_status ★★★
// ═══════════════════════════════════════════════════════
async function initDatabase() {
    const client = await pool.connect();
    try {
        // Main message buffer — FIX #1: uuid is UNIQUE, used for ON CONFLICT
        await client.query(`
        ...
        // ★ FIX #4: Add ack_status column if missing (migration safety)
        try {
        ...
        console.log('✅ Database initialized (V15-STABLE)');
...
// ═══════════════════════════════════════════════════════
// ★★★ FIX #1: addToBuffer with ON CONFLICT ★★★
// ═══════════════════════════════════════════════════════
async function addToBuffer(type, data, senderSocketId, clientUuid, clientHash) {
    ...
    if (dbAvailable) {
        try {
            // ★ FIX #1: ON CONFLICT (uuid) DO UPDATE — prevents unique constraint errors
            await pool.query(
            ...
            // ★ FIX #1: If seq conflict too, try with a new seq
...
// ═══════════════════════════════════════════════════════
// ★★★ FIX #3: getBufferAfterSeq — for missed data ★★★
// ═══════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════
// ★★★ FIX #4: markMessageAcked — full ACK by uuid ★★★
// ═══════════════════════════════════════════════════════
...
// ★ FIX #5: Move to dead letter queue
...
async function cleanOldMessages() {
    ...
    try {
        ...
        // Clean old dead letters (>30 days)
...
// ═══════════════════════════════════════════════════════════════════
// ★★★ FIX #6: Helper — broadcast to all EXCEPT sender ★★★
// ═══════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════
// ★★★ Socket.IO — V15-STABLE + FIX #6: No echo back ★★★
// ═══════════════════════════════════════════════════════
io.on('connection', async (socket) => {
    ...
    socket.emit('welcome', {
        ...
        pending_ack: pendingCount,
        db_available: dbAvailable,
        server_version: 'V15-STABLE'
    });
    socket.emit('machine_status', { status: machineStatus });

    // ═══════════════════════════════════════════════════
    // ★★★ FIX #3: get_missed_messages — recovery ★★★
    // ═══════════════════════════════════════════════════
    ...
    // ═══ sync_request (kept for backward compat) ═══
    ...
    socket.on('ack_message', async (payload) => {
        ...
        // ★ FIX #6: Send updated pending count to all clients EXCEPT sender
        const pendingCount = await getPendingCount();
        broadcastExceptSender('pending_count_update', { pending: pendingCount }, socket.id);
        // Also send to the sender (they need the count too, but this is informational not data)
        socket.emit('pending_count_update', { pending: pendingCount });
    });
    ...
    // ═══════════════════════════════════════════════════
    // ★★★ Command handling — FIX #6: No echo back ★★★
    // ═══════════════════════════════════════════════════
    socket.on('command', async (payload) => {
        ...
        if (['START', 'STOP', 'PAUSE'].includes(action)) {
            ...
            console.log(`📡 Broadcasting machine_command [${action}] to others [seq=${entry.seq}]`);

            // ★ FIX #6: Broadcast to all EXCEPT the sender
            const machineData = {
            ...
            broadcastExceptSender('update_ui', updateUiData, socket.id);
        ...
        if (action === 'data_change') {
            ...
            console.log(`📡 Broadcasting data_changed [${changeType}] to others [seq=${entry.seq}]`);
            ...
            broadcastExceptSender('data_changed', dataChangedPayload, socket.id);
    ...
    // ═══════════════════════════════════════════════════════════════
    // ★★★ Direct event handling — FIX #6: No echo back ★★★
    // ═══════════════════════════════════════════════════════════════
    ...
    async function handleDirectEvent(eventName, data, socket) {
        ...
        // ★ FIX #1: Dedup by UUID — check before insert
        ...
        socket.emit('server_ack', {
            seq: entry.seq,
            uuid: clientUuid || entry.uuid,
            server_uuid: entry.uuid,
            original_action: eventName, status: 'stored_in_db'
        });

        console.log(`   ✅ ${eventName} [seq=${entry.seq}, uuid=${entry.uuid.substring(0, 8)}...] stored+broadcast (sender excluded)`);
    }

    // ★ FIX #5: dead_letter event — client reports a dead message
...
async function startServer() {
    ...
    server.listen(PORT, () => {
        ...
        console.log(`🚀 Server V15-STABLE ready on port ${PORT}`);
        console.log(`💾 PostgreSQL: ${dbAvailable ? '✅' : '❌'}`);
        console.log(`🔐 UUID + Server-side Hash`);
        console.log(`📡 FIX #6: broadcastExceptSender() — no echo back to sender`);
        console.log(`🔁 NACK/Resend + Dead Letter Queue`);
        console.log(`📬 FIX #1: ON CONFLICT — no more unique constraint errors`);
        console.log(`🔌 FIX #2: pingTimeout=120s, pingInterval=25s`);
        console.log(`🔄 FIX #3: get_missed_messages event`);
        console.log(`✅ FIX #4: ack_message → pending counter → 0`);
        console.log(`💀 FIX #5: Dead letter queue for failed messages`);
        console.log(`🚫 FIX #6: No broadcast echo back to original sender`);
        console.log('═'.repeat(65));
    });
}
