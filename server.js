...
// ═══════════════════════════════════════════════════════
// ★★★ V14-FIX: Hash متوافق تماماً مع Python ★★★
// ═══════════════════════════════════════════════════════
...
/**
 * ★ V14-FIX: حساب Hash باستخدام JSON.stringify الأصلي
 * بدل محاولة محاكاة Python — السيرفر يحسب hash خاص به
 * والعملاء يثقون بـ hash السيرفر عند الاستلام
 *
 * التصميم الجديد:
 * - المُرسل يحسب hash محلي ويرسله
 * - السيرفر يُخزّن البيانات ويحسب hash خاص (server_hash)
 * - المستلم يتحقق من hash السيرفر (الذي حسبه السيرفر من البيانات المخزنة)
 * - لا حاجة لتطابق hash بين Python و Node.js
 */
function computeServerHash(data) {
    const str = JSON.stringify(data, Object.keys(data).sort());
    return crypto.createHash('sha256').update(str, 'utf8').digest('hex');
}

/**
 * ★ V14-FIX: Hash ثابت ومتسق — يُستخدم فقط داخل السيرفر
 * العملاء يتحققون من بيانات السيرفر باستخدام hash السيرفر
 */
...
// ★ V14-FIX: إصلاح updateMachineState — embroidery في المكان الصحيح
async function updateMachineState(status, data) {
    ...
    if (data) {
        ...
        if (data.embroidery) lastData.embroidery = data.embroidery;  // ★ FIX: كان color
...
io.on('connection', async (socket) => {
    ...
    socket.on('nack_resend', async (payload) => {
        ...
        if (uuid) {
            ...
            if (msg) {
                // ★ V14-FIX: إعادة حساب hash السيرفر من البيانات المخزنة
    ...
    socket.on('command', async (payload) => {
        ...
        if (['START', 'STOP', 'PAUSE'].includes(action)) {
            ...
            // ★ V14-FIX: نرسل client_uuid الأصلي حتى يتعرف عليه TX
    ...
    async function handleDirectEvent(eventName, data, socket) {
        ...
        // ★ V14-FIX: نرسل hash السيرفر + UUID الأصلي
        const broadcastData = {
            ...cleanData,
            seq: entry.seq, uuid: entry.uuid, hash: entry.hash
        };
        ...
        // ★ V14-FIX: ACK يحتوي على client UUID الأصلي
        socket.emit('server_ack', {
            seq: entry.seq,
    ...
    socket.on('speed_update', async (data) => {
        ...
        socket.broadcast.emit('speed_update', broadcastData);
    });
...
async function startServer() {
    ...
    server.listen(PORT, () => {
        ...
        console.log(`🚀 السيرفر V14-FIX جاهز على المنفذ ${PORT}`);
        console.log(`💾 PostgreSQL: ${dbAvailable ? '✅' : '❌'}`);
        console.log(`🔐 UUID + Server-side Hash (no cross-language hash issues)`);
